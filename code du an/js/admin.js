...
async function adminDeleteTour(btn) {
  ...
  try {
    var res = await apiAdminDeleteTour(id);
    if (res && res.ok) {
...
// ============================================================
async function adminLoadCoupons(page) {
  page = page || 1;
  var res = await apiAdminGetCoupons({ page: page, limit: 10 });
  if (!res || !res.ok) { showToast('Không thể tải mã giảm giá'); return; }
  var coupons = res.data?.result?.coupons || res.data?.result || [];
  var body = document.getElementById('promoListBody');
  if (!body) return;
  if (!coupons.length) {
    body.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:24px;color:var(--muted)">Chưa có mã giảm giá</td></tr>';
    return;
  }
  body.innerHTML = coupons.map(function (c) {
    return '<tr>' +
      '<td>' + (c.code || '—') + '</td>' +
      '<td>' + (c.value ? c.value.toLocaleString('vi-VN') + 'đ' : '—') + '</td>' +
      '<td>' + (c.used_count || 0) + '/' + (c.max_usage || '∞') + '</td>' +
      '<td>' + (c.expires_at ? new Date(c.expires_at).toLocaleDateString('vi-VN') : '—') + '</td>' +
      '<td><span style="color:' + (c.is_active ? 'var(--green)' : '#e55') + '">' + (c.is_active ? 'Đang dùng' : 'Tắt') + '</span></td>' +
      '<td><button onclick="adminToggleCoupon('' + c._id + '')" style="background:none;border:1px solid #ddd;padding:4px 10px;border-radius:6px;cursor:pointer;font-size:0.78rem">' + (c.is_active ? 'Tắt' : 'Bật') + '</button></td>' +
        '</tr>';
  }).join('');
}
...
async function adminToggleCoupon(id) {
  ...
  if (res && res.ok) { showToast('Đã cập nhật trạng thái mã giảm giá'); adminLoadCoupons(); }
}

async function adminCreateCoupon(data) {
  var res = await apiAdminCreateCoupon(data);
  if (res && res.ok) { showToast('Tạo mã giảm giá thành công'); adminLoadCoupons(); }
  else showToast('Lỗi tạo mã giảm giá');
}
...
// QUẢN LÝ TOUR (Admin)
// ============================================================
async function adminCreateTour(formData) {
  var res = await apiAdminCreateTour(formData);
  if (res && res.ok) { showToast('Tạo tour thành công'); return res.data?.result; }
  showToast('Lỗi tạo tour: ' + (res?.data?.message || ''));
  return null;
}

async function adminUpdateTour(id, formData) {
  var res = await apiAdminUpdateTour(id, formData);
  if (res && res.ok) { showToast('Cập nhật tour thành công'); return true; }
  showToast('Lỗi cập nhật tour'); return false;
}

async function adminDeleteTour(id) {
  if (!confirm('Xóa tour này?')) return;
  var res = await apiAdminDeleteTour(id);
  if (res && res.ok) showToast('Đã xóa tour');
  else showToast('Không thể xóa — tour còn đơn đặt');
}

async function adminUpdateTourStatus(id, status) {
  var res = await apiAdminUpdateTourStatus(id, status);
  if (res && res.ok) showToast('Cập nhật trạng thái thành công');
}

// ============================================================
// QUẢN LÝ LỊCH KHỞI HÀNH (Admin)
// ============================================================
async function adminCreateSchedule(tourId, data) {
  var res = await apiAdminCreateSchedule(tourId, data);
  if (res && res.ok) { showToast('Tạo lịch khởi hành thành công'); return true; }
  showToast('Lỗi tạo lịch khởi hành'); return false;
}

async function adminUpdateSchedule(id, data) {
  var res = await apiAdminUpdateSchedule(id, data);
  if (res && res.ok) { showToast('Cập nhật lịch thành công'); return true; }
  showToast('Lỗi cập nhật lịch'); return false;
}

async function adminDeleteSchedule(id) {
  if (!confirm('Xóa lịch này?')) return;
  var res = await apiAdminDeleteSchedule(id);
  if (res && res.ok) showToast('Đã xóa lịch khởi hành');
  else showToast('Không thể xóa — còn booking liên quan');
}