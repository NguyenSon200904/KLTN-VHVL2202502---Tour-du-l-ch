...
// ============================================================
// CẬP NHẬT TRẠNG THÁI BOOKING (Nhân viên)
// ============================================================
async function nvUpdateStatus() {
  var sel = document.getElementById('nvModalStatusSelect');
  var id = sel?.dataset?.bookingId;
  var status = parseInt(sel?.value);
  if (!id || isNaN(status)) return;

  var res = await apiAdminUpdateBookingStatus(id, status);
  if (res && res.ok) {
    showToast('Cập nhật trạng thái thành công');
    if (typeof nvRenderBookings === 'function') nvRenderBookings();
    document.getElementById('nvModal').style.display = 'none';
    if (typeof nvUpdateUrgentCount === 'function') nvUpdateUrgentCount();
  } else {
