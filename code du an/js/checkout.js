...
window.addEventListener('DOMContentLoaded', async function () {
  // Smart Defaulting: gợi ý điểm khởi hành theo vị trí
  applySmartDefault();
...
// Mapping tọa độ → thành phố Việt Nam
const CITY_COORDS = [
  { name: 'Hà Nội', lat: 21.028, lng: 105.834 },
  { name: 'TP. Hồ Chí Minh', lat: 10.776, lng: 106.700 },
  { name: 'Đà Nẵng', lat: 16.054, lng: 108.202 },
  { name: 'Cần Thơ', lat: 10.045, lng: 105.747 },
  { name: 'Hải Phòng', lat: 20.845, lng: 106.688 },
  { name: 'Huế', lat: 16.463, lng: 107.590 },
  { name: 'Nha Trang', lat: 12.238, lng: 109.196 },
  { name: 'Đà Lạt', lat: 11.940, lng: 108.435 },
  { name: 'Quy Nhơn', lat: 13.776, lng: 109.223 },
  { name: 'Vinh', lat: 18.665, lng: 105.692 },
]
...
function getNearestCity(lat, lng) {
  var nearest = CITY_COORDS[0], minDist = Infinity
  CITY_COORDS.forEach(function (city) {
    var d = getDistance(lat, lng, city.lat, city.lng)
    if (d < minDist) { minDist = d; nearest = city }
  })
  return nearest.name
}

function smartDefaultDepartureCity(callback) {
  if (!navigator.geolocation) { if (callback) callback(null); return }

  navigator.geolocation.getCurrentPosition(
    function (pos) {
      var city = getNearestCity(pos.coords.latitude, pos.coords.longitude)
      // Lưu vào localStorage để dùng lại
      localStorage.setItem('vt_user_city', city)
      if (callback) callback(city)
    },
    function () {
      // Nếu từ chối vị trí → dùng city đã lưu hoặc null
      var saved = localStorage.getItem('vt_user_city')
...
function applySmartDefault() {
  // Lấy thành phố đã lưu trước (nhanh, không cần hỏi lại)
  var savedCity = localStorage.getItem('vt_user_city')

  // Tìm input điểm khởi hành
  var departureInputs = document.querySelectorAll(
    'input[placeholder*="khởi hành"], input[placeholder*="xuất phát"], input[id*="depart"], input[id*="Depart"]'
  )

  if (savedCity && departureInputs.length) {
    departureInputs.forEach(function (inp) {
      if (!inp.value) {
        inp.value = savedCity
        inp.style.borderColor = 'var(--green)'
        // Thêm badge gợi ý
        var badge = document.createElement('div')
        badge.style.cssText = 'font-size:0.72rem;color:var(--green);margin-top:3px;display:flex;align-items:center;gap:4px'
        badge.innerHTML = '📍 Dựa trên vị trí của bạn'
        inp.parentNode.appendChild(badge)
        setTimeout(function () { badge.style.opacity = '0'; badge.style.transition = 'opacity 1s' }, 3000)
      }
    })
  }

  // Hỏi vị trí nếu chưa có
  if (!savedCity) {
    smartDefaultDepartureCity(function (city) {
      if (!city) return
      departureInputs.forEach(function (inp) {
        if (!inp.value) inp.value = city
      })
    })
  }
}